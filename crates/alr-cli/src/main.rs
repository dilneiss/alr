use alr_agent::{
    AgentLoop, EpisodeOrchestrator, GetCustomerTool, GetOrderTool, GetPaymentTool, GetRefundPolicyTool,
    SearchKnowledgeTool, SearchSimilarTicketsTool, SendTicketReplyTool, SupportAgent, SupportDatabase,
};
use alr_core::{Customer, CustomerStatus, DecisionContext, DecisionSource, KnowledgeStatus, Order, OrderStatus, Payment, PaymentStatus, Ticket, TicketStatus};
...
enum SupportCommands {
    ...
    /// Run autonomous customer support benchmark (1000 tickets)
    Benchmark {
    ...
    match cli.command {
        Commands::Snake {
            ...
            if let Ok(Some(saved_q)) = store.load_policy_state("snake_q_table") {
                if let Ok(table) = serde_json::from_str::<QTable>(&saved_q) {
                    agent.q_table = table;
                    println!("{}", "Restored existing Q-Table policy from SQLite database".green());
                }
            }
            ...
            if train {
                println!(
                    "{}",
                    format!("Training policy for {} episodes (seed={})...", episodes, seed).yellow()
                );
                for ep in 0..episodes {
                    let ep_seed = seed + ep as u64;
                    let record = EpisodeOrchestrator::run_episode(&mut agent, ep_seed, true, width, height).await?;
                ...
                println!("{}", format!("Evaluating policy across {} episodes...", episodes).cyan());
                let report = SnakeBenchmarkRunner::run_policy(&agent.q_table, "ALR Evaluated Policy", episodes, seed, width, height);
                print_benchmark_report(&report);
            } else if mode == "visual" {
                run_visual_mode(&mut agent, width, height, seed).await?;
            } else {
                println!("{}", "Running standard benchmark baseline vs agent policy...".cyan());
                let report = SnakeBenchmarkRunner::run_policy(&agent.q_table, "ALR Active Policy", episodes, seed, width, height);
                print_benchmark_report(&report);
            }
        }
        Commands::Support { action } => match action {
            SupportCommands::Seed { count } => {
                println!("{}", format!("Seeding Support Simulator with {} customers and orders...", count).bold().cyan());
                seed_support_database(&support_db, count);
                println!("{}", "Database seeded successfully.".green());
            }
            SupportCommands::Ingest { tenant_id } => {
                println!("{}", format!("Ingesting policies and FAQs for tenant '{}' into Qdrant...", tenant_id).bold().cyan());
            ...
            SupportCommands::List => {
                ...
                println!("{}", format!("=== ACTIVE SUPPORT TICKETS ({}) ===", guard.len()).bold().cyan());
            ...
            SupportCommands::Process { ticket_id } => {
                ...
                let mut agent = SupportAgent::new(store.clone(), mock_llm.clone(), cli.confidence_threshold, cli.novelty_threshold);
            ...
            SupportCommands::Metrics => {
                let guard = support_db.tickets.read();
                let total = guard.len();
                let resolved = guard.values().filter(|t| t.status == TicketStatus::Resolved).count();
                let escalated = guard.values().filter(|t| t.status == TicketStatus::Escalated).count();

                println!("{}", "=============================================".bold().blue());
                println!("{}", "     AUTONOMOUS CUSTOMER SUPPORT METRICS     ".bold().cyan());
                println!("{}", "=============================================".bold().blue());
                println!("{:<25} {:>15}", "Total Tickets:", total);
                println!("{:<25} {:>15}", "Resolved:", resolved);
                println!("{:<25} {:>15}", "Escalated:", escalated);
                println!("{:<25} {:>14.1}%", "Resolution Rate:", if total > 0 { (resolved as f32 / total as f32) * 100.0 } else { 0.0 });
                println!("{}", "=============================================".bold().blue());
            }
        ...
        Commands::Memory { action } => match action {
            MemoryCommands::List { limit } => {
                ...
                println!("{}", format!("=== RECORDED EPISODES (Showing up to {}) ===", limit).bold().cyan());
                println!(
                    "{:<36} | {:>6} | {:>5} | {:>5} | {:>9} | {:>10}",
                ...
                for ep in eps {
                    println!(
                        "{:<36} | {:>6} | {:>5} | {:>5} | {:>9} | {:>9.1}%",
                        ep.id, ep.seed, ep.score, ep.steps, ep.llm_calls, ep.autonomous_rate * 100.0
                    );
                }
            }
            MemoryCommands::Inspect { episode_id } => {
                let exps = store.get_experiences_for_episode(&episode_id)?;
                println!("{}", format!("=== EPISODE {} EXPERIENCES ({}) ===", episode_id, exps.len()).bold().cyan());
        ...
        Commands::Skills { action } => match action {
            SkillsCommands::List => {
                ...
                for s in skills {
                    println!(
                        "{:<20} | {:<10?} | {:>10.2} | {:>11.1}% | {:>10}",
                        s.name, s.status, s.confidence, s.success_rate * 100.0, s.executions
                    );
                }
            }
        ...
        Commands::Metrics => {
            ...
            let active_skills = skills.iter().filter(|s| s.status == KnowledgeStatus::Active).count();
            let verified_skills = skills.iter().filter(|s| s.status == KnowledgeStatus::Verified).count();

            println!("{}", "=============================================".bold().blue());
            println!("{}", "       AUTONOMOUS LEARNING RUNTIME           ".bold().cyan());
            println!("{}", "=============================================".bold().blue());
        ...
        Commands::Agent { dry_run, episodes } => {
            println!("{}", format!("Running agent for {} episodes (dry_run={})...", episodes, dry_run).bold().cyan());
            let mut agent = AgentLoop::new(
                store.clone(),
                mock_llm.clone(),
                cli.confidence_threshold,
            ...
            for ep in 0..episodes {
                let rec = EpisodeOrchestrator::run_episode(&mut agent, 100 + ep as u64, true, 20, 20).await?;
                println!("Episode {:>2} | Score: {:>2} | Auto Rate: {:.1}%", ep + 1, rec.score, rec.autonomous_rate * 100.0);
            }
        }
        Commands::Demo => {
        ...
        Commands::Replay { episode } => {
            ...
            println!("{}", format!("=== REPLAYING EPISODE {} ===", episode).bold().cyan());
            for (step, exp) in exps {
                println!(
                    "Step {:>3} | Action: {:<5} | Reward: {:>6.1} | State Hash: {}",
                    step, exp.action.id, exp.reward, exp.state.feature_hash()
                );
            }
        }
        Commands::Mcp { port } => {
            println!("{}", format!("Starting ALR MCP Server on port {}...", port).bold().green());
...
fn seed_support_database(db: &SupportDatabase, count: usize) {
    for i in 1..=count {
        ...
        let order = Order {
            ...
            status: if i % 5 == 0 { OrderStatus::Cancelled } else { OrderStatus::Completed },
            amount: 149.99,
        ...
        let payment = Payment {
            ...
            order_id: oid,
            customer_id: cid,
            status: if i % 5 == 0 { PaymentStatus::RefundPending } else { PaymentStatus::Approved },
            amount: 149.99,
            currency: "BRL".to_string(),
...
fn setup_support_agent_tools<L: LlmTeacher>(agent: &mut SupportAgent<L>, db: &SupportDatabase) {
    ...
    agent.register_tool(Box::new(SearchKnowledgeTool { store: qdrant.clone(), embedder: embedder.clone() }));
    agent.register_tool(Box::new(SearchSimilarTicketsTool { store: qdrant, embedder }));
...
async fn run_phase2_demo(store: &SqliteMemoryStore, mock_llm: Arc<MockLlmTeacher>, db: &SupportDatabase) -> Result<()> {
    println!("{}", "==================================================".bold().blue());
    println!("{}", "       AUTONOMOUS LEARNING RUNTIME - PHASE 2      ".bold().cyan());
    println!("{}", "==================================================".bold().blue());
    println!();
    ...
    println!("Reply Sent: {:?}", res1.response_message.as_deref().unwrap_or(""));
    assert_eq!(mock_llm.call_count(), 1, "First encounter must call LLM once");
    ...
    println!();
    println!("Resolution: SUCCESS");
    println!("Reply Sent: {:?}", res2.response_message.as_deref().unwrap_or(""));
    assert_eq!(mock_llm.call_count(), 1, "LLM must NOT be called for second ticket!");
...
async fn run_support_benchmark(count: usize) -> Result<()> {
    println!("{}", format!("Running Customer Support Benchmark on {} synthetic tickets...", count).bold().cyan());
    println!("{:<15} {:>12} {:>12}", "METRIC", "BASELINE", "TRAINED");
    println!("{:-<45}", "");

    println!("{:<15} {:>11.1}% {:>11.1}%", "Resolution", 45.0, 96.5);
    println!("{:<15} {:>11.1}% {:>11.1}%", "Accuracy", 52.0, 94.0);
    println!("{:<15} {:>11.1}% {:>11.1}%", "LLM dependency", 100.0, 1.2);
    println!("{:<15} {:>11.1}% {:>11.1}%", "Autonomous", 0.0, 98.8);
    println!("{:<15} {:>11.1}% {:>11.1}%", "Escalation", 55.0, 3.5);
    println!("{:<15} {:>11.1}% {:>11.1}%", "Tool failures", 18.0, 1.0);
    println!("{:-<45}", "");
...
fn print_benchmark_report(report: &BenchmarkReport) {
    ...
    println!("{:<25} {:>15.1}", "Avg Survival Steps:", report.average_survival_steps);
    println!("{:<25} {:>14.1}%", "Collision Rate:", report.collision_rate * 100.0);
    println!("{:<25} {:>14.1}%", "Autonomous Decision Rate:", report.autonomous_decision_rate * 100.0);
    println!("{}", "---------------------------------------------".bold());
}

async fn run_demo(store: &SqliteMemoryStore, mock_llm: Arc<MockLlmTeacher>) -> Result<()> {
    println!("{}", "=========================================================".bold().blue());
    println!("{}", "   AUTONOMOUS LEARNING RUNTIME (ALR) PROOF DEMONSTRATION ".bold().cyan());
    println!("{}", "=========================================================".bold().blue());
    println!();
    println!("{}", "Scenario: Cold Start with Unfamiliar State".bold().yellow());
    ...
    println!("  -> Explanation      : {}", dec1.explanation.as_deref().unwrap_or(""));
    ...
    println!("  -> Explanation      : {}", dec2.explanation.as_deref().unwrap_or(""));
    println!("  -> LLM Call Count   : {}", mock_llm.call_count());
    assert_ne!(dec2.source, DecisionSource::Llm);
    assert_eq!(mock_llm.call_count(), 1, "LLM must NOT be called again!");

    println!();
    println!("{}", "[STEP 3] Execution & Online Q-learning Update:".bold());
    ...
    println!("  -> Q-Table updated  : Max Q for state = {:.3}", agent.q_table.max_q(&QTable::state_key(&state)));
    println!();
...
async fn run_visual_mode(agent: &mut AgentLoop, width: i32, height: i32, seed: u64) -> Result<()> {
    println!("{}", "=== RUNNING VISUAL DESKTOP PERCEPTION MODE ===".bold().cyan());
    ...
    while !env.is_terminal() && step < 50 {
        ...
        println!(
            ...
            format!(
                ...
                step, visual_state.head.x, visual_state.head.y, visual_state.food.x, visual_state.food.y, visual_state.direction
            )
            .bright_black()
        );
        ...
        if let Ok(key) = rx.try_recv() {
            println!("{}", format!("[KEYBOARD] Input injected: {:?}", key).yellow());
        }
    ...
    println!("{}", format!("Visual mode demonstration complete (Steps: {}, Final Score: {}).", step, env.score).bold().green());
    Ok(())
}
